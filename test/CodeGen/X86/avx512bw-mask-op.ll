...
; RUN: llc < %s -mtriple=x86_64-linux-gnux32 -mcpu=skx | FileCheck %s
...
; CHECK:       {{##|#}} BB#0:
; CHECK-NEXT:    kmovd ([[ARG1:%rdi|%edi]]), %k0
; CHECK-NEXT:    knotd %k0, %k0
; CHECK-NEXT:    kmovd %k0, ([[ARG1]])
; CHECK-NEXT:    retq
...
; CHECK:       {{##|#}} BB#0:
; CHECK-NEXT:    kmovq (%{{r|e}}di), %k0
; CHECK-NEXT:    knotq %k0, %k0
; CHECK-NEXT:    kmovq %k0, (%{{r|e}}di)
; CHECK-NEXT:    retq
...
; CHECK:       {{##|#}} BB#0:
; CHECK-NEXT:    kmovd (%{{r|e}}di), %k0
; CHECK-NEXT:    kmovd (%{{r|e}}si), %k1
